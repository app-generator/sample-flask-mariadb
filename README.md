...
- MariaDB python connector : to test the connector, the easy way is run a MariaDB service, then in the .env file change DEBUG to False and modify the DB_* variables at convenience except for DB_ENGINE. The connector was successfully tested using Slackware 15 running a remote MariaDB server (version 10.8) docker image in the same subnetwork.
